    ...
    <div class="col-xs-12 col-md-8">
        <div class="panel panel-default">
            <div class="panel-body">
                <form id='date-filter-form' action="{!! route('laravel-controller-audit.index.post') !!}" method="post"
                       class="form-inline pull-right">
                    {!! csrf_field() !!}
                    <div class="form-group">
                        <select name="user_id" class="form-control">
                            <option>Any User</option>
                            @foreach($active_users_composer as $user)
                                <option value="{!! $user->id !!}" @if(isset($user_id) && $user_id == $user->id) selected="selected" @endif>{!! $user->name !!}</option>
                            @endforeach
                        </select>
                    </div>
                    ...
                    <div class="form-group">
                        <select name="prefix_date" id="prefix_date" class="form-control js-prefix-date">
                            ...
                            <option data-start="{!! \Carbon\Carbon::now()->previous(\Carbon\Carbon::SUNDAY)->subDays(1)->startOfDay()->format('Y-m-d') !!}"
                                    data-end="{!! \Carbon\Carbon::now()->previous(\Carbon\Carbon::SUNDAY)->endOfDay()->format('Y-m-d') !!}">
                                Last Weekend
                            </option>
                    ...
                    <div class="form-group">
                        <input type="submit" class="btn btn-primary" value="Get Audits">
                    </div>
                </form>
            </div>
        </div>
